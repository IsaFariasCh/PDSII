...
// Atualizações pós instalção da programação defensiva
#ifndef JOGO_HPP
#define JOGO_HPP
...
public:
    ...
    inline static void clear_screen_jogo()
    {
        ...
        system("clear"); // Comando para limpar a tela no Linux/Unix
#endif
        cout << "A tela foi limpa. Insira uma letra se já deseja continuar ou 0 para retornar ao menu: ";
        char pause;
        cin >> pause;
        if (pause == '0')
        {
            throw std::runtime_error("Returning to menu");
        }
#ifdef _WIN32 // Verifica se estamos no Windows
        system("cls"); // Comando para limpar a tela no Windows
#else
