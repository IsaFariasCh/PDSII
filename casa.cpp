...
void Casa::preencher_jogadores()
{
    int Quantidade_de_Jogadores_Local = this->Quantidade_de_jogadores;

    for (int iterador = 0; iterador < Quantidade_de_Jogadores_Local; iterador++)
    {
        Set_Quantidade_de_jogadores_Jogador_Entrou(true);
    }
    IJogo::clear_screen_jogo(1);
}
...
{
    char jogoo;
    cout << "Selecione os jogos disponíveis" << endl << "(Somente o primeiro símbolo do nome)" << endl;

    if (Quantidade_de_jogadores == 1)
    {
        cout << "Para 1 jogador temos:" << endl;
        cout << "Paciência(P)" << endl;
    }
    else if (Quantidade_de_jogadores == 2)
    {
        cout << "Para 2 jogadores temos:" << endl;
    ...
    while (1)
    {
        ...
        case '2':
            return 'v';
        default:
            cout << "Inválido! Selecione novamente." << endl;
            break;
        }
    }
}
...
void Casa::explica_games()
{
    ...
    cout << endl << endl << "Insira qualquer coisa para continuar..." << endl;
    char pausa;
    cin >> pausa;
}
...
void Casa::Set_Quantidade_de_jogadores_Jogador_Entrou(bool nao_decrementar)
{
    cout << "Qual o nome do novo jogador?" << endl;
    std::string nomee;
    cin >> nomee;
...
void Casa::Set_Quantidade_de_jogadores_Jogador_Saiu()
{
    ...
    std::string nomee;
    cin >> nomee;
    const int tamanho_vector = this->jogadores.size();
    int jogador_inexistente = 0;
    ...
    for (int iteracao = 0; iteracao < tamanho_vector; iteracao++)
    {
        if (nomee == jogadores[iteracao].Get_nome())
        {
            this->jogadores.erase(jogadores.begin() + iteracao);
            this->Quantidade_de_jogadores--;
            break;
        }
        jogador_inexistente++;
    }
    // A variável jogador_inexistente foi incrementada sempre que o for não encerrou após encontrar o jogador, então se a variável for do tamanho do vetor significa que ele não achou o jogador, então o jogador não existe.
    if (jogador_inexistente == tamanho_vector)
    {
...
void Casa::Set_Quantidade_de_jogadores()
{
    ...
    while (1)
    {
        ...
        if (Aumentar_ou_Diminuir == 'A')
        {
            this->Set_Quantidade_de_jogadores_Jogador_Entrou();
            break;
        }
        else if (Aumentar_ou_Diminuir == 'D')
        {
            this->Set_Quantidade_de_jogadores_Jogador_Saiu();
            break;
        }
        else
        {
